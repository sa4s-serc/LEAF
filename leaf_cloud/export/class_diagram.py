...
from leaf_cloud.core.petri_net import PetriNet, Place, Token, Transition, Arc
...
def default_resource_mapping_function(
    ...
    for place_id, place in model_builder.petri_net.places.items():
        ...
        for res_id, resource in model_builder.resource_mapping.items():
            ...
            res_name = (
                resource.name if hasattr(resource, "name") else str(res_id)
            )
...
class ClassDiagramGenerator:
    ...
    def __init__(
        self,
        model: Union[PetriNet, ModelBuilder, Dict[str, Any]],
        ...
        if isinstance(model, PetriNet):
            ...
            raise ValueError(
                "Model must be a PetriNet, ModelBuilder, or dictionary"
            )
        ...
        self.bottlenecks = (
            self._identify_bottlenecks() if include_bottlenecks else []
        )

        logger.info(
            f"ClassDiagramGenerator initialized with {len(self.resources)} resources"
        )

    def _extract_resources_from_petri_net(
        ...
        for place in petri_net.places.values():
            ...
            if isinstance(place_resource_attr, Resource):
                ...
                if isinstance(place_resource_attr.resource_type, ResourceType):
                    ...
                    logger.warning(
                        f"Place {place.id}'s resource {res_name} has unexpected resource_type: {type(place_resource_attr.resource_type)}"
                    )
                ...
                logger.warning(
                    f"Place {place.id} has a 'resource' attribute of unexpected type: {type(place_resource_attr)}. Skipping."
                )
    ...
    def _analyze_relationships(self) -> List[Tuple[str, str, RelationType]]:
        ...
        place_to_resource: Dict[str, str] = {
            res.name: res_id for res_id, res in self.resources.items()
        }
            ...
            for in_arc in input_arcs:
                ...
                for out_arc in output_arcs:
                    ...
                    relationships.append(
                        (src_res, tgt_res, RelationType.CONNECTS_TO)
                    )
        return relationships
                ...
                for source, target, _ in list(self.relationships):
                    if target == resource_id:
                        new_relations.append(
                            (source, target, RelationType.BOTTLENECK)
                        )
    ...
    def generate_mermaid_code(self) -> str:
        ...
        for (
            ...
            ResourceType.__members__.values()
        ):  # Iterate over enum member instances
        ...
        for res_id, resource_item in self.resources.items():
            ...
            if isinstance(resource_item, Resource):
                ...
                if isinstance(resource_item.resource_type, ResourceType):
                    ...
                    logger.warning(
                        f"Resource object {res_id} has unexpected resource_type type: {type(resource_item.resource_type)}. Defaulting to GENERIC."
                    )
                ...
                res_type_val = resource_item.get(
                    "type", ResourceType.GENERIC.value
                )
                ...
                logger.warning(
                    f"Item {res_id} in self.resources is of unexpected type: {type(resource_item)}. Using default values."
                )
                res_type_str = ResourceType.GENERIC.value
            ...
            if res_type_str in resource_by_type:
                ...
                logger.warning(
                    f"Resource type '{res_type_str}' for {res_id} not in predefined categories. Adding to GENERIC."
                )
                resource_by_type[ResourceType.GENERIC.value].append(
                    (res_id, res_name_str)
                )
        ...
        mermaid_lines.append(
            "    classDef bottleneck fill:#ff6666,stroke:#333,stroke-width:4px"
        )
    ...
    def export_to_file(self, output_path: str, format: str = "png") -> str:
        ...
        try:
            ...
            logger.warning(
                "Mermaid CLI not found. Consider installing with 'npm install -g @mermaid-js/mermaid-cli'"
            )
            ...
            with open(output_path, "w") as f:
                f.write(f"```mermaid\n{mermaid_code}\n```")
            logger.info(
                f"Saved Mermaid code to {output_path} - manually render with a Mermaid-compatible tool"
            )
...
def model_builder_to_class_diagram(
    model_builder: ModelBuilder,
    output_path: str,
    format: str = "png",
    include_bottlenecks: bool = True,
