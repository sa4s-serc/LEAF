...
import subprocess
import base64
from leaf_cloud.core.petri_net import PetriNet
...
class DeploymentDiagramGenerator:
    ...
    def __init__(
        self, petri_net: PetriNet, resource_mapping: Dict[str, Resource]
    ):
        """
            ...
            for in_arc in input_arcs:
                ...
                for out_arc in output_arcs:
                    ...
                    if transition is None:
                        logger.warning(
                            f"Transition not found for ID: {trans_id}"
                        )
                        continue
    ...
    def generate_mermaid_diagram(self) -> str:
        ...
        for source, target, transition in self.connections:
            ...
            transition_label = (
                transition.name.split("_")[-1]
                if "_" in transition.name
                else ""
            )
    ...
    def export_data_flow(self, output_path: str, format: str = "svg") -> str:
        ...
        for source, target, _ in self.connections:
            if (
                source.resource_type == ResourceType.COMPUTE
                or target.resource_type == ResourceType.COMPUTE
            ):
