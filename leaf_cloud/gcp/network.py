...
class NetworkResource(Resource):
    ...
    def utilization_ratio(self) -> float:
        """Derive network utilization from current throughput vs bandwidth.

        Falls back to base calculation (allocated/total) if throughput/bandwidth
        are not available.
        """
        try:
            bw = float(getattr(self, "bandwidth_mbps", 0.0) or 0.0)
            th = float(getattr(self, "current_throughput_mbps", 0.0) or 0.0)
            if bw > 0:
                return max(0.0, min(1.0, th / bw))
        except Exception:
            pass
    ...
    def get_power_consumption(self, timestamp: float) -> float:
        """
        Calculate power consumption at a given timestamp.
        ...
        Args:
            timestamp: Simulation time. Currently unused in this model but kept for API consistency.

        Returns:
            Power consumption in kW.
        """
        # Base power consumption is modeled as a percentage of max power
        base_power_ratio = 0.2

        # Get current utilization (based on traffic)
        utilization = self.utilization_ratio

        # Calculate power factor: base power + utilization-dependent portion
        power_factor = base_power_ratio + (1 - base_power_ratio) * utilization

        # Scale by bandwidth capacity. Assume 10W (0.01kW) per Gbps at max utilization for a standard resource.
        power_per_gbps_kw = 0.01
        bandwidth_gbps = self.bandwidth_mbps / 1000

        # Different network resources have different energy profiles (resource_factor)
        if self.network_type == NetworkResourceType.VPC:
            ...
            resource_factor = (
                1.2  # CDNs use more energy due to distributed nature
            )
        ...
        # Final power calculation in kW
        power_kw = (
            power_factor * bandwidth_gbps * resource_factor * power_per_gbps_kw
        )
...
class VPC(NetworkResource):
    ...
    def __init__(
        ...
        super().__init__(
            ...
            latency_ms=0.5,
        )  # VPC has very low base latency

        self.subnet_ranges = subnet_ranges or []
        self.auto_create_subnets = auto_create_subnets
        self.mtu = mtu
        # resource_id -> resource_type
        self.connected_resources: Dict[str, str] = {}

    def add_connected_resource(
        ...
        logger.debug(
            f"Added resource {resource_id} of type {resource_type} to VPC {self.name}"
        )
    ...
    def get_power_consumption(self, timestamp: float) -> float:
        ...
        """
        base_energy = super().get_power_consumption(timestamp)
...
class CDN(NetworkResource):
    ...
    def __init__(
        ...
        super().__init__(
            ...
            bandwidth_mbps=bandwidth_mbps,
            latency_ms=15.0,
        )  # CDNs have higher latency due to geographic distribution
...
class Interconnect(NetworkResource):
    ...
    def __init__(
        ...
        super().__init__(
            ...
            bandwidth_mbps=bandwidth_mbps,
            latency_ms=5.0,
        )  # Direct interconnects have low latency
...
class VPN(NetworkResource):
    ...
    def __init__(
        ...
        super().__init__(
            ...
            bandwidth_mbps=bandwidth_mbps,
            latency_ms=20.0,
        )  # VPNs add encryption overhead to latency
    ...
    def add_tunnel(
        self, tunnel_name: str, shared_secret: Optional[str] = None
    ) -> None:
        """Add a VPN tunnel"""
        self.tunnels.append(
            {"name": tunnel_name, "shared_secret": shared_secret}
        )

    def add_remote_peer(
        self, peer_ip: str, peer_name: Optional[str] = None
    ) -> None:
...
class APIGateway(NetworkResource):
    ...
    def __init__(
        ...
        super().__init__(
            ...
            bandwidth_mbps=bandwidth_mbps,
            latency_ms=2.0,
        )  # API Gateway adds ~2ms of latency
